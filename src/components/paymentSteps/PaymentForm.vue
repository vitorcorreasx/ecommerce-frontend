...
import { useCartStore, useCardStore } from '../../store';

const userCard = useCardStore();
  ...
  <div class="container row q-pl-xl justify-between">
    <div class="column">
      ...
      <div class="table q-mt-xl">
        <q-card>
          <q-tabs v-model="tab" dense class="text-grey" active-color="brown-10" indicator-color="brown-10" align="justify"
            narrow-indicator>
          ...
          <q-tab-panels v-model="tab" animated>
            <q-tab-panel name="card" class="row">
              <q-input v-model="cardNumber" outlined class="col-6 q-mr-md q-mb-md" label="Número do Cartão"
                mask="#### #### #### ####"
                :rules="[val => val && val.length > 0 || 'Digite um múmero de cartão válido']" />
              <q-input v-model="cvv" outlined class="col-2 q-mr-md q-mb-md" label="CVC" mask="###"
                :rules="[val => val && val.length > 0 || 'Digite o CVC conforme cartão']" />
              <q-input v-model="valiteDate" outlined class="q-mb-md" label="Validade (MM/AAAA)" mask="##/####"
                :rules="[val => val && val.length > 0 || 'Digite a validade conforme cartão']" />
              <q-input v-model="name" outlined class="col-6 q-mr-md " label="Nome Títular"
                :rules="[val => val && val.length > 0 || 'Digite o nome conforme cartão']" />
              <q-input v-model="cpf" outlined class="col-5" label="CPF" mask="###.###.###-##"
